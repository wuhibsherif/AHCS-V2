...
def add_medication_details(request, pk, pk2):
    ...
    if request.method == 'POST':
        medication_detail_form = Medication_Form(request.POST)
        patient = Patient.objects.get(id=pk)
        ...
        if medication_detail_form.is_valid():
            print("valid")
            medication_detail_form.save_medication_detail(context)
            # nxt = request.POST.get('next', '/')
            return redirect('prescription_detail_url', patient.basic_id)
        else:
            print(medication_detail_form.errors)
    else:
...
def prescription_detail(request, pk):
    ...
    print(prescription)
    context = {'user_profile': patient, 'found': True, 'medication': medication,
